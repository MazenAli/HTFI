...
DimensionIndexIterator<Iterator1D>::DimensionIndexIterator(const DimensionIndex &_idx, const int _dim, const int _min, const int _max):idx(_idx), dim(_dim), minit(_min), maxit(_max) {
		assert(idx.length() > _dim - 1 && _dim > 0);
}

DimensionIndexIterator<Iterator1D>::DimensionIndexIterator(const DimensionIndexIterator<Iterator1D> &iter):idx(iter.getIndex()), dim(iter.getDim()), minit(iter.getMin()), maxit(iter.getMax()){}
...
DimensionIndexIterator<IteratorXD>::DimensionIndexIterator(const DimensionIndex & _idx, const DimensionIndex & _activeDims, const DimensionIndex & _min, const DimensionIndex & _max):idx(_idx),  minit(_min), maxit(_max), active(_activeDims){
	assert(maxit.length() == idx.length() && minit.length() == idx.length());
}

DimensionIndexIterator<IteratorXD>::DimensionIndexIterator(const DimensionIndexIterator<IteratorXD> &iter):idx(iter.getIndex()),minit(iter.getMin()), maxit(iter.getMax()),active(iter.getActiveDims()){
	assert(maxit.length() == idx.length() && minit.length() == idx.length());
}
...
DimensionIndexIterator<IteratorXDALL>::DimensionIndexIterator(const DimensionIndex & _idx, const DimensionIndex & _min, const DimensionIndex & _max):idx(_idx), minit(_min), maxit(_max) {
	assert(maxit.length() == idx.length() && minit.length() == idx.length());
}

DimensionIndexIterator<IteratorXDALL>::DimensionIndexIterator(const DimensionIndexIterator<IteratorXDALL> &iter):idx(iter.getIndex()),minit(iter.getMin()),maxit(iter.getMax()){
	assert(maxit.length() == idx.length() && minit.length() == idx.length());
}
