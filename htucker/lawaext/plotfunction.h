...
namespace lawa{
	...
	plotfunction(const BasisFunction<_T,_Side,_Domain,_Cons> * BF, const int j, const long k, const unsigned short deriv, int steps = 100){
		...
		flens::DenseVector<flens::Array< _T> > x = flens::linspace(BF->support(j,k).l1,BF->support(j,k).l2,steps+1);
		...
		cout << x.firstIndex() << "  " << x.lastIndex() << endl;
		cout << y.firstIndex() << " " << y.lastIndex() << endl;
		std::ofstream plotFileT("plotBasisFunction.tmp");
		for(int i = 1; i <= x.length(); ++i){
			plotFileT << x(i-1) << " " << y(i) << endl;
		} 
		plotFileT.close();
