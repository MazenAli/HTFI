...
void test_htuckerclosure(){
	MatrixTensor<SparseGeMatrix<CRS<double,CRS_General> > > mt(2);

	SparseGeMatrix<CRS<double, CRS_General> > A(3,3);
	A(1,1) = 1;
	A(2,2) = 2;
	A(3,3) = 3;
	A.finalize();
	mt.setMatrix(1,A);
	A.~SparseGeMatrix<CRS<double, CRS_General> >();
	SparseGeMatrix<CRS<double,CRS_General> > B(2,2);
	B(1,2) = 1.5;
	B(2,1) = 3;
	B.finalize();
	cout << B << endl;
